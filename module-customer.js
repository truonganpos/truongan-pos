    ...
    try {
        ...
        if(s) {
            filtered = filtered.filter(c => {
                let name = String(c['Tên khách hàng'] || c['Tên khách hàng'] || '').toLowerCase(); let phone = cleanPhone(c['Điện thoại']); let cusId = String(c['Mã khách hàng'] || '').toLowerCase(); 
        ...
        let html = viewMode === 'table' ? `<div class="table-responsive"><table>
            <thead>
                <tr>
                    ...
                    <th class="th-cus-name" title="Tên khách hàng">Tên khách hàng</th>
        ...
        sliced.forEach(c => {
            try {
                ...
                let cName = String(c['Tên khách hàng'] || c['Tên khách hàng'] || '---'); let cNameTitle = cName.replace(/"/g, '&quot;');
...
function openCustomerModal(phone = '') {
    ...
    if(phone) {
        ...
        if(cus) {
            if(op) op.value = phone; if(ep) ep.value = phone; if(en) en.value = cus['Tên khách hàng'] || cus['Tên khách hàng'] || '';
...
function openPayDebtModal(phone) {
    ...
    document.getElementById('payDebtName').innerText = cus['Tên khách hàng'] || cus['Tên khách hàng'] || phone;
